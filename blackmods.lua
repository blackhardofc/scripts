...
-- 🔑 CHAVE: BLACK2026
-- 🛡️ PROTEÇÕES ANTI-BAN + TODAS AS FUNÇÕES REAIS
-- 🎨 Painel Futurista | Arrastável | Redimensionável
-- =============================================

-- ⚠️ AVISO: Nenhum anti-ban é 100% garantido. Use com consciência.

-- =============================================
-- 🛡️ SISTEMA DE PROTEÇÕES ANTI-BAN
-- =============================================
local Protecoes = {}
...
local function GetAlvo()
    ...
    for _, v in pairs(workspace:GetChildren()) do
        if v:FindFirstChild("HumanoidRootPart") and v:FindFirstChild("Humanoid") and v ~= Player.Character and v.Humanoid.Health > 0 then
            local Pos, Vis = Camera:WorldToScreenPoint(v.HumanoidRootPart.Position)
            if Vis then
...
task.spawn(function()
    while task.wait(0.08) do
        if Funcoes.TriggerBot and UIS:IsMouseButtonDown(Funcoes.TriggerKey) then
            local Alvo = GetAlvo()
            if Alvo then
                game:GetService("ReplicatedStorage").Remotes.Throw:FireServer()
            end
        end
    end
end)
        ...
        if Funcoes.HitboxAumentada then
            for _, v in pairs(workspace:GetChildren()) do
                if v:FindFirstChild("HumanoidRootPart") and v ~= Player.Character then
                    pcall(function()
                        v.HumanoidRootPart.Size = Vector3.new(6,8,6)
                    end)
                end
            end
        end
...
-- 🔄 SEM RESFRIAMENTO E LANÇAMENTO RÁPIDO
task.spawn(function()
...
function AbrirPainel()
    ...
    Painel.Parent = Tela
    Painel.BackgroundColor3 = Color3.fromRGB(8, 10, 18)
    ...
    CriarBotao("🎯 MODO CABEÇA", Funcoes, "ModoCabeça")
